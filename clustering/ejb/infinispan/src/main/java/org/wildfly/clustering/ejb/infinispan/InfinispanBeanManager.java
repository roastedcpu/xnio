...
public class InfinispanBeanManager<I, T> implements BeanManager<I, T, TransactionBatch> {
    ...
    public void start() {
        ...
        if ((timeout != null) && !timeout.isNegative()) {
            if (this.dispatcherFactory.getGroup().isSingleton()) {
                ...
                schedulers.add(new BeanExpirationScheduler<>(
                        this.batcher, this.beanFactory, this.expiration));
            }
        }
        ...
        if ((this.dispatcher != null) && ((dist == null) || !dist.getWriteConsistentHash().getPrimarySegmentsForOwner(this.cache.getCacheManager().getAddress()).isEmpty())) {
    ...
    private static class ScheduleExpirationTask<I> implements Runnable {
        ...
        ScheduleExpirationTask(Cache<BeanKey<I>, BeanEntry<I>> cache, Predicate<Map.Entry<? super BeanKey<I>, ? super BeanEntry<I>>> filter, Scheduler<I> scheduler, Locality oldLocality,Locality newLocality) {
            this.cache = cache;
            this.filter = filter;
    ...
    private class SchedulableBean implements Bean<I, T> {
        ...
        public void close() {
            ...
            if (!this.bean.isValid()) {
                InfinispanBeanManager.this.cancel(this.getId());
            }
            else {
