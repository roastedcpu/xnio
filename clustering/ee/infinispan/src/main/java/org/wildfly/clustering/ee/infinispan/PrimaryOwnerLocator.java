...
public class PrimaryOwnerLocator<K> implements Function<K, Node> {
    ...
    public PrimaryOwnerLocator(Cache<K, ?> cache, NodeFactory<Address> memberFactory, Group group) {
        ...
        // Invalidation caches map all keys to a single segment, thus keys should use local member
        this.distribution = mode.needsStateTransfer() ? cache.getAdvancedCache().getDistributionManager() : null;
